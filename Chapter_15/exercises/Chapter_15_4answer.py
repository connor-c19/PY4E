...
except: print("Tables already exist. Moving on...")
...
for entry in json_data:
     ...
     # stored the role column in the Member table to complete the assignment.
...
query_check_roster = """SELECT User.name,Course.title, Member.role FROM 
    User JOIN Member JOIN Course 
    ON User.id = Member.user_id AND Member.course_id = Course.id
    ORDER BY User.name DESC, Course.title DESC, Member.role DESC LIMIT 2;"""


result_check_roster = cur.execute(query_check_roster).fetchall()
print(result_check_roster)
...
query_find_user = """SELECT 'XYZZY' || hex(User.name || Course.title || Member.role ) AS X FROM 
    User JOIN Member JOIN Course 
    ON User.id = Member.user_id AND Member.course_id = Course.id
    ORDER BY X LIMIT 1;"""

result_find_user = cur.execute(query_find_user).fetchall()
print(result_find_user)
    
#one row with a string that looks like XYZZY53656C696E613333.
...
## NOTES ##
# Desired input: 
# Desired output: one row with a string that looks like XYZZY53656C696E613333.
